...
class FormData {
  ...
  expandUploads() {
    const uploadsEntry = this.getEntriesByName(_internalAttributes.default.uploadInfoHiddenInputName)[0];
  ...
  parseBody(body) {
    ...
    for (var _iterator = lines, _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
      ...
      } else if (state === ParserState.inPreamble) bufferUtils.appendLine(this.preamble, line);else if (state === ParserState.inHeaders) {
        if (line.length) currentEntry.setHeader(line.toString());else state = ParserState.inBody;
      } else if (state === ParserState.inEpilogue) bufferUtils.appendLine(this.epilogue, line);else if (state === ParserState.inBody) bufferUtils.appendLine(currentEntry.body, line);
    }
  }
