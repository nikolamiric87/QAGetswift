...
const SVG_XLINK_HREF_TAGS = ['animate', 'animateColor', 'animateMotion', 'animateTransform', 'mpath', 'set', 'linearGradient', 'radialGradient', 'stop', 'a', 'altglyph', 'color-profile', 'cursor', 'feimage', 'filter', 'font-face-uri', 'glyphref', 'image', 'mpath', 'pattern', 'script', 'textpath', 'use', 'tref'];
...
class DomProcessor {
  ...
  processElement(el, urlReplacer) {
    ...
    for (let i = 0; i < this.elementProcessorPatterns.length; i++) {
      ...
      if (pattern.selector(el) && !this._isShadowElement(el)) {
        for (let j = 0; j < pattern.elementProcessors.length; j++) pattern.elementProcessors[j].call(this, el, urlReplacer, pattern); // @ts-ignore
  ...
  _isOpenLinkInIframe(el) {
    ...
    if (target !== '_top') {
      const mustProcessTag = DomProcessor.isIframeFlagTag(tagName) || DomProcessor._isHtmlImportLink(tagName, rel);
  ...
  _processScriptElement(script) {
    ...
    if (isExecutableScript) {
      ...
      result = commentPrefix + (0, _script.processScript)(result, true, false) + commentPostfix;
      if (hasCDATA) result = '\n//<![CDATA[\n' + result + '//]]>';
  ...
  _processUrlAttrs(el, urlReplacer, pattern) {
    if (urlReplacer && pattern.urlAttr) {
      ...
      const processedOnServer = this.adapter.hasAttr(el, storedUrlAttr); // NOTE: Page resource URL with proxy URL.
      ...
      if ((resourceUrl || resourceUrl === '') && !processedOnServer) {
        if (urlUtils.isSupportedProtocol(resourceUrl) || isSpecialPage) {
          ...
          let proxyUrl = '';
          const charsetAttrValue = isScript && this.adapter.getAttr(el, 'charset'); // NOTE: Only a non-relative iframe src can be cross-domain.
          ...
          if (isIframe && !isSpecialPage && !isRelativePath) {
            ...
            if (!parsedResourceUrl.protocol) resourceUrl = proxyUrlObj.destResourceInfo.protocol + resourceUrl; // NOTE: Cross-domain iframe.
            ...
            if (!this.adapter.sameOriginCheck(destUrl, resourceUrl)) {
              ...
              const iframeResourceType = urlUtils.getResourceTypeString({
                isIframe: true
              });
