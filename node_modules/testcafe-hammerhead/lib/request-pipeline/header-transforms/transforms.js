...
var _authorization = _interopRequireDefault(require("../xhr/authorization"));
...
function isCrossDomainXhrWithoutCredentials(ctx) {
  return ctx.isXhr && !ctx.req.headers[_headers.default.withCredentials] && ctx.dest.reqOrigin !== ctx.dest.domain;
}

function transformAuthorizationHeader(src, ctx) {
  if (src.includes(_authorization.default.valuePrefix)) return src.replace(_authorization.default.valuePrefix, '');
  return isCrossDomainXhrWithoutCredentials(ctx) ? void 0 : src;
}

function transformCookieForFetch(src, ctx) {
  const requestCredentials = ctx.req.headers[_headers.default.fetchRequestCredentials];
...
function generateSyncCookie(ctx, parsedServerCookies) {
  ...
  if (ctx.parsedClientSyncCookie) {
    const outdatedSyncCookies = ctx.parsedClientSyncCookie.actual.filter(clientCookie => {
      ...
      for (var _iterator = parsedServerCookies, _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
        var _ref;
...
function resolveAndGetProxyUrl(url, ctx) {
  ...
  const _parseUrl = (0, _url2.parse)(url),
        host = _parseUrl.host;
...
const requestTransforms = Object.assign({
  ...
  [_headers.default.requestMarker]: skip,
  [_headers.default.withCredentials]: skip,
  [_headers.default.origin]: skip,
  [_headers.default.fetchRequestCredentials]: skip
}, _authorization.default.headers.reduce((obj, header) => {
...
const forcedResponseTransforms = {
  'set-cookie': (src, ctx) => {
    let parsedCookies;
    if (src) parsedCookies = ctx.session.cookies.setByServer(ctx.dest.url, src);
    if (!ctx.isPage || ctx.isIframe) return generateSyncCookie(ctx, parsedCookies || []);
    return [];
  }
};
